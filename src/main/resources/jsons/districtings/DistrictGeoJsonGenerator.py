...
with open('./{}_precincts_data.json'.format(districtingId), 'r+') as districts_file:
    district_data = json.load(districts_file)

if district_data["State"] == "LA":
    precincts_data = geopandas.read_file("../precincts/LouisianaPrecincts.json")
elif district_data["State"] == "PA":
    precincts_data = geopandas.read_file("../precincts/PennsylvaniaPrecincts.json")
else:
    precincts_data = geopandas.read_file("../precincts/CaliforniaPrecincts.json")
...
with open('./{}_districts_data.json'.format(districtingId), 'w+') as outfile:
