    ...
    render() {
        ...
        return(
            ...
            <Map ref = {this.map} center={[40.0, -98]} zoom={5} scrollWheelZoom={false} minZoom={5} maxZoom={5} dragging={false} doubleClickZoom={false}>
                ...
                {this.state.stateSelected === "CA" && this.state.caPrecinct !== "" && this.props.precinctsIsSet ? 
					<GeoJSON data={this.state.caPrecinct} className={precinctClass} style={this.precinctStyle}></GeoJSON> :
					null
				}
                {console.log(this.state.stateSelected === "LA" && this.props.precinctsIsSet)}
                {this.state.stateSelected === "LA" && this.state.laPrecinct !== "" && this.props.precinctsIsSet ? 
					<GeoJSON data={this.state.laPrecinct} className={precinctClass} style={this.precinctStyle}></GeoJSON>:
                    null
                }
                {this.state.stateSelected === "PA" && this.state.paPrecinct !== "" && this.props.precinctsIsSet ? 
					<GeoJSON data={this.state.paPrecinct} className={precinctClass} style={this.precinctStyle}></GeoJSON> :
					null
				}
                {/* GeoJSON for State */}
                <GeoJSON ref = {this.stateGeoJson} data={stateGeoJson} className={mapClass} style={this.stateStyle} onEachFeature={this.onEachFeature}></GeoJSON>
